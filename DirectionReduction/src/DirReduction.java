...
Going to one direction and coming back the opposite direction is a needless effort. Since this is
the wild west, with dreadfull weather and not much water, it's important to save yourself some
...
directly opposite but they become directly opposite after the reduction of "EAST" and
...
The Clojure version returns nil when the path is reduced to nothing. The Rust version takes a
slice of enum Direction {NORTH, SOUTH, EAST, WEST}.
...
public class DirReduction {
    ...
    public static String[] dirReduc(String[] arr) {
        String[] ret;
        Map.Entry<String, Integer> north = Map.entry("North", 0);
        Map.Entry<String, Integer> east = Map.entry("East", 0);

        for(String s:arr) {
            if(s.contains("North")) north.setValue(north.getValue() + 1);
            if(s.contains("South")) north.setValue(north.getValue() - 1);
            if(s.contains("East")) east.setValue(east.getValue() + 1);
            if(s.contains("West")) east.setValue(east.getValue() - 1);
        }
